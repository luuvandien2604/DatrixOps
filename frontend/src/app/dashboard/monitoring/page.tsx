                      ...
                      <td className="py-4 px-6 text-right">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          <button
                            ...
                            className="p-1.5 bg-rose-500/10 hover:bg-rose-500/20 rounded border border-rose-500/20 text-rose-400 hover:text-rose-300 transition-colors" title="Reboot VPS">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="glass-card w-full max-w-md bg-[#0B0F14] border-rose-500/30 overflow-hidden flex flex-col">
            <div className="flex items-center gap-3 p-6 border-b border-white/5 bg-rose-500/5">
              ...
              <h2 className="text-xl font-bold text-[var(--foreground)]">Reboot VPS?</h2>
            </div>
            <div className="p-6">
              <p className="text-[var(--color-muted)] mb-4">
                You are about to <strong className="text-rose-400">reboot the entire VPS</strong> <strong className="text-[var(--foreground)]">{serverToRestart.name}</strong>. All services running on this server (not just the agent) will be interrupted until the machine finishes rebooting.
              </p>
              <div className="mb-6">
              ...
              <div className="flex justify-end gap-3">
                ...
                <button
                  disabled={confirmRestartText !== serverToRestart.name || isRestarting}
                  onClick={async () => {
                    try {
                      ...
                      alert(`Reboot command sent to ${serverToRestart.name}. The server will be back online in a few minutes.`);
                    } catch (err) {
                  ...
                  {isRestarting ? 'Rebooting...' : 'Reboot VPS'}
                </button>
              </div>
            </div>
          </div>
