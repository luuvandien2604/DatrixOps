  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[var(--foreground)] tracking-tight">Alerts</h1>
      </div>

      <div role="tablist" aria-label="Alert configuration" className="flex gap-4 border-b border-[var(--border-color)]">
        <button type="button" role="tab" aria-selected={activeTab === 'rules'} onClick={() => setActiveTab('rules')} className={`pb-3 text-sm font-medium transition-colors ${activeTab === 'rules' ? 'text-blue-500 border-b-2 border-blue-500' : 'text-[var(--color-muted)] hover:text-[var(--foreground)]'}`}>Rules</button>
        <button type="button" role="tab" aria-selected={activeTab === 'channels'} onClick={() => setActiveTab('channels')} className={`pb-3 text-sm font-medium transition-colors ${activeTab === 'channels' ? 'text-blue-500 border-b-2 border-blue-500' : 'text-[var(--color-muted)] hover:text-[var(--foreground)]'}`}>Channels</button>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1 bg-[var(--background-card)] border border-[var(--border-color)] rounded-xl p-5">
            ...
            <form onSubmit={createRule} className="space-y-4">
              <div>
                <label htmlFor="rule-name" className="block text-sm text-[var(--color-muted)] mb-1">Alert name</label>
                <input id="rule-name" name="rule-name" required value={ruleName} onChange={e => setRuleName(e.target.value)} type="text" className="w-full bg-transparent border border-[var(--border-color)] rounded-lg p-2 text-sm text-white" placeholder="Example: High CPU usage" />
              </div>
              <div>
                <label htmlFor="rule-metric" className="block text-sm text-[var(--color-muted)] mb-1">Metric</label>
                <select id="rule-metric" name="rule-metric" value={ruleMetric} onChange={e => setRuleMetric(e.target.value)} className="w-full bg-transparent border border-[var(--border-color)] rounded-lg p-2 text-sm text-white">
                  ...
                  <option value="status" className="bg-[#0B0F14]">Offline status</option>
                </select>
              </div>
              {ruleMetric !== 'status' && (
                <div className="flex gap-2">
                  <div className="flex-1">
                    <label htmlFor="rule-operator" className="block text-sm text-[var(--color-muted)] mb-1">Condition</label>
                    <select id="rule-operator" name="rule-operator" value={ruleOperator} onChange={e => setRuleOperator(e.target.value)} className="w-full bg-transparent border border-[var(--border-color)] rounded-lg p-2 text-sm text-white">
                      <option value=">" className="bg-[#0B0F14]">Greater than (&gt;)</option>
                      <option value="<" className="bg-[#0B0F14]">Less than (&lt;)</option>
                    </select>
                  </div>
                  <div className="flex-1">
                    <label htmlFor="rule-threshold" className="block text-sm text-[var(--color-muted)] mb-1">Threshold (%)</label>
              ...
              <div key={rule.id} className="bg-[var(--background-card)] border border-[var(--border-color)] rounded-xl p-4 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-[var(--color-muted)] mt-1">
                    {rule.metric === 'status' ? 'Alert when server is offline' : `${rule.metric.toUpperCase()} ${rule.operator} ${rule.threshold}%`}
                  </p>
                </div>
                <button type="button" aria-label={`Delete rule ${rule.name}`} onClick={() => deleteRule(rule.id)} className="text-rose-500 hover:text-rose-400 p-2"><Trash2 className="w-5 h-5"/></button>
              </div>
            ))}
            {rules.length === 0 && <div className="p-8 text-center text-[var(--color-muted)] border border-[var(--border-color)] rounded-xl border-dashed">No alert rules yet.</div>}
          </div>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1 bg-[var(--background-card)] border border-[var(--border-color)] rounded-xl p-5">
            ...
            <form onSubmit={createChannel} className="space-y-4">
              <div>
                <label htmlFor="channel-name" className="block text-sm text-[var(--color-muted)] mb-1">Channel name</label>
                <input id="channel-name" name="channel-name" required value={channelName} onChange={e => setChannelName(e.target.value)} type="text" className="w-full bg-transparent border border-[var(--border-color)] rounded-lg p-2 text-sm text-white" placeholder="Example: IT Support" />
              </div>
              <div>
                <label htmlFor="channel-type" className="block text-sm text-[var(--color-muted)] mb-1">Platform</label>
          ...
          <div className="md:col-span-2 space-y-4">
            {channels.map(ch => (
              <div key={ch.id} className="bg-[var(--background-card)] border border-[var(--border-color)] rounded-xl p-4 flex justify-between items-center">
                ...
                <button type="button" aria-label={`Delete channel ${ch.name}`} onClick={() => deleteChannel(ch.id)} className="text-rose-500 hover:text-rose-400 p-2"><Trash2 className="w-5 h-5"/></button>
              </div>
            ))}
            {channels.length === 0 && <div className="p-8 text-center text-[var(--color-muted)] border border-[var(--border-color)] rounded-xl border-dashed">No notification channels yet.</div>}
          </div>
        </div>
      )}
    </div>
