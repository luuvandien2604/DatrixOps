...
Browser → Frontend (Next.js, :3000) → Backend API (Go net/http, :8080) → PostgreSQL
                              ...
                              gửi heartbeat mặc định mỗi 5s → /api/v1/agent/heartbeat
```
...
2. Sau đó mỗi `DATRIXOPS_INTERVAL` giây (mặc định **5s**) → gửi heartbeat cơ bản (CPU/RAM/Net/Disk).
...
6. Task types hỗ trợ: `docker_start`, `docker_stop`, `docker_restart`, `docker_logs`, `service_start`, `service_stop`, `service_restart`, `service_reload`, `agent_update`, `agent_restart`, `vps_reboot`.
7. Backend trả `update_available` và `latest_version` khi version lệch. `update_required` luôn false để agent legacy không tự update ngoài ý muốn.
8. Task `agent_update` tải signed manifest, xác minh Ed25519, size, SHA-256, executable/version marker đúng OS/architecture, stage/replace binary rồi để service manager restart. Không chạy lại installer nên giữ nguyên token và service configuration.

### ⚠️ Bug/gotcha đã biết khi test local
Agent gọi `{ServerURL}/agent/heartbeat`, KHÔNG tự thêm `/api/v1`. Các installer production set `DATRIXOPS_SERVER_URL=https://host/api/v1`, còn default trong `agent/internal/config/config.go` là `http://localhost:8080` (thiếu `/api/v1`). Test local phải set thủ công:
```bash
