...
func (r *Repository) DeleteRule(ctx context.Context, id, userID string) error {
	tx, err := r.db.Pool.Begin(ctx)
	if err != nil {
		return fmt.Errorf("begin delete alert rule transaction: %w", err)
	}
	defer func() { _ = tx.Rollback(ctx) }()

	var ownedRuleID string
	if err := tx.QueryRow(ctx, `
		SELECT id
		FROM alert_rules
		WHERE id = $1 AND user_id = $2
		FOR UPDATE
	`, id, userID).Scan(&ownedRuleID); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
	...
	// Clean up notifications generated by this rule
	if _, err := tx.Exec(ctx, `DELETE FROM dashboard_notifications WHERE alert_rule_id = $1 AND user_id = $2`, id, userID); err != nil {
		return fmt.Errorf("clean alert rule notifications: %w", err)
	}

	// Clean up active alert state & incidents for this rule
	if _, err := tx.Exec(ctx, `DELETE FROM alert_state WHERE rule_id = $1`, id); err != nil {
		return fmt.Errorf("clean alert rule state: %w", err)
	}
	...
	if err := tx.Commit(ctx); err != nil {
		return fmt.Errorf("commit delete alert rule: %w", err)
	}
	return nil
}
...
// MarkNotificationRead đánh dấu một notification là đã xem.
...
// MarkAllNotificationsRead đánh dấu toàn bộ notification chưa xem của workspace.
